...
Public Class Inventory_manage
    ...
    Sub General_inv_cal()
        ...
        '========================================
        constituents = New DataTable
        constituents.Columns.Add("part_name", GetType(String))
        constituents.Columns.Add("demand_assem", GetType(String))
        ...
        Try
            ...
            For i = 0 To dimen_table.Rows.Count - 1
                ...
                '----------------------------------------------------------------------------------------------------------------------------------
                '----------- this will process an assemblu constitues and insert them in consttuents table along with the demand ------------------
                If my_assemblies.Contains(dimen_table.Rows(i).Item(0)) = True Then

                    Dim checki As New MySqlCommand
                    checki.Parameters.Clear()
                    checki.Parameters.AddWithValue("@Legacy_ADA", dimen_table.Rows(i).Item(0))
                    checki.CommandText = "select Part_Name, Qty from adv where Legacy_ADA = @Legacy_ADA"
                    checki.Connection = Login.Connection
                    checki.ExecuteNonQuery()

                    Dim reader3 As MySqlDataReader
                    reader3 = checki.ExecuteReader

                    If reader3.HasRows Then

                        While reader3.Read

                            '--- check if the parts has already been added otherwise add the demand
                            Dim add_con As Boolean : add_con = True

                            For j = 0 To constituents.Rows.Count - 1
                                If String.Equals(constituents.Rows(j).Item(0).ToString, reader3(0).ToString) = True Then
                                    constituents.Rows(j).Item(1) = constituents.Rows(j).Item(1) + demand
                                    add_con = False
                                    Exit For
                                End If
                            Next

                            If add_con = True Then
                                constituents.Rows.Add(reader3(0).ToString, reader3(1).ToString * demand)
                            End If
            ...
            For i = 0 To constituents.Rows.Count - 1


                Try
                    Dim exist_c As Boolean : exist_c = False
                    Dim totalc As Double : totalc = 0
                    Dim min_qty As Double : min_qty = 0
                    Dim max_qty As Double : max_qty = 0
                    Dim current_qty As Double : current_qty = 0
                    Dim in_transit As Double : in_transit = 0
                    Dim demand_c As Double : demand_c = 0
                    Dim desc As String : desc = ""

                    Dim check_cmd2 As New MySqlCommand
                    check_cmd2.Parameters.Clear()
                    check_cmd2.Parameters.AddWithValue("@part_name", constituents.Rows(i).Item(0))
                    check_cmd2.CommandText = "select part_name, min_qty, max_qty, current_qty, Qty_in_order, description from inventory.inventory_qty where part_name = @part_name and current_qty <  min_qty"
                    ...
                    If reader2.HasRows Then
                        While reader2.Read
                            ...
                            min_qty = reader2(1)
                            max_qty = reader2(2)
                            current_qty = reader2(3)
                            in_transit = reader2(4)
                            desc = reader2(5)
                        End While
                    End If
                    ...
                    If exist_c = True Then
                        ...
                        'max - current_inv - coming_parts
                        totalc = max_qty - current_qty - in_transit
                        demand_c = cal_demand(constituents.Rows(i).Item(0)) + constituents.Rows(i).Item(1)

                        totalc = totalc + demand_c

                        'insert into material order table 
                        If totalc > 0 And ((current_qty + in_transit - demand_c) < min_qty = True) Then   'after second AND is (on hnad, qty_transit - demand) >= min


                            Dim exist_i As Boolean : exist_i = False


                            '--check if part exist---

                            Dim cmdq As New MySqlCommand
                            cmdq.Parameters.AddWithValue("@Part_No", constituents.Rows(i).Item(0))
                            cmdq.CommandText = "SELECT * from inventory.Material_orders where Part_No = @Part_No"
                            ...
                            If exist_i = True Then

                                Dim main_cmd As New MySqlCommand
                                main_cmd.Parameters.Clear()
                                main_cmd.Parameters.AddWithValue("@Part_No", constituents.Rows(i).Item(0))
                                main_cmd.Parameters.AddWithValue("@Qty_needed", If(totalc < 0, 0, totalc))

                                main_cmd.CommandText = "Update inventory.Material_orders set Qty_needed = @Qty_needed where Part_No = @Part_No"
                                main_cmd.Connection = Login.Connection
                                main_cmd.ExecuteNonQuery()
                            Else

                                Dim Create_cmdi As New MySqlCommand
                                Create_cmdi.Parameters.Clear()
                                Create_cmdi.Parameters.AddWithValue("@Part_No", constituents.Rows(i).Item(0))
                                Create_cmdi.Parameters.AddWithValue("@description", desc)
                                Create_cmdi.Parameters.AddWithValue("@Qty_needed", If(totalc < 0, 0, totalc))

                                Create_cmdi.CommandText = "INSERT INTO inventory.Material_orders(Part_No, description,  Qty_needed) VALUES (@Part_No, @description, @Qty_needed)"
