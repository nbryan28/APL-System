...
Public Class Procurement_Overview
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        ElseIf TabControl1.SelectedTab Is TabPage1 Then
            ...
            For i = 0 To orders_grid.Rows.Count - 1
                ...
                If orders_grid.Rows(i).Cells(0).Value.ToString.Contains(TextBox1.Text) Then
                    orders_grid.CurrentCell = orders_grid.Rows(i).Cells(0)
                    Exit For
                End If
            Next
    ...
    'Private Sub MoveToPackiNgSlipToolStripMenuItem_Click(sender As Object, e As EventArgs)
    ...
    Private Sub mpl_grid_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles mpl_grid.CellValueChanged

        If go_ahead = True Then

            For Each row As DataGridViewRow In mpl_grid.Rows
                If row.IsNewRow Then Continue For

                If IsNumeric(row.Cells(6).Value) = False Then
                    row.Cells(6).Value = 0
                End If



                If (IsNumeric(row.Cells(0).Value) = True And IsNumeric(row.Cells(6).Value)) Then


                    row.Cells(5).Value = If(IsNumeric(row.Cells(0).Value) = False, 0, row.Cells(0).Value) - If(IsNumeric(row.Cells(6).Value) = False, 0, row.Cells(6).Value)


                    If CType(row.Cells(5).Value, Double) <> 0 Then
                        row.Cells(5).Style.BackColor = Color.Firebrick
                    Else
                ...
                If IsNumeric(row.Cells(7).Value) = True Then
                    If row.Cells(7).Value > row.Cells(5).Value Then
                        row.Cells(7).Value = row.Cells(5).Value
                    End If
                Else
                    row.Cells(7).Value = 0
                End If
